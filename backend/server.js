...
const mongoUrl =
  process.env.MONGO_URL || "mongodb://localhost/diagnostisDiabetes"
...
app.post("/symptoms", async (req, res) => {
  const { username, age, gender, risk, items, checkedItems, parameters } =
    req.body
  ...
  try {
    ...
    if (user && bcrypt.compareSync(password, user.password)) {
      ...
      res
        ...
        .json({
          success: false,
          message: "User not found. Please set up account",
        })
    }
  } catch (error) {
    res.status(400).json({ success: false, message: "Invalid request", error })
